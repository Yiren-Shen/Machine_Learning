...
def buildGraph():
    ...
    y_target=tf.placeholder(tf.int32,[None,10])
    Lambda = tf.placeholder("float32", name='Lambda')

    # Graph definition
    h1,W1=build_block(X,1000)
    x2=tf.nn.relu(h1)
    h2, W2 = build_block(x2, 10)
    ...
    return X,y_target, Lambda, train, y_predict,loss_train
...
X, y_target,Lambda,train,y_predict,loss_train = buildGraph()
...
## Training hyper-parameters
    ...
    for step in xrange(0,epho*np.int(numBatches)):
        ...
        #acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(trainTarget[i*B:(i+1)*B], 1), tf.argmax(y_trainhat, 1)), "float"))
        acc=np.mean(np.equal(np.argmax(trainTarget[i*B:(i+1)*B], 1), np.argmax(y_trainhat, 1),dtype=np.float32))
        i +=1
        #
        trainLoss_list.append(err_train)
        trainAcc_list.append(acc)
        if (step+1) % numBatches == 0:
           # randIdx = np.arange(len(trainData))
           #np.random.shuffle(randIdx)
           # trainData = trainData[randIdx]
           i=0
           print("Iter :%3d, MSE-train: %4.2f,Acc: %4.2f " % (step, err_train, acc))