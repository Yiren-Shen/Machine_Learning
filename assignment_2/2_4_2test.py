...
def buildGraph():
    ...
    loss_valid = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=tf.add(tf.matmul(tf.add(tf.matmul(X_valid, W1), b1), W2), b2),
    ...
    return keep_prob, X, y_target, X_valid, y_valid, X_test, y_test, Lambda, train, y_predict, loss_train, y_predict_valid, y_predict_test, loss_valid, loss_test, W1, W2
...
keep_prob, X, y_target, X_valid, y_valid, X_test, y_test, Lambda, train, y_predict, loss_train, y_predict_valid, y_predict_test, loss_valid, loss_test, W1, W2 = buildGraph()
...
with tf.Session() as sess:
    ...
    for step in xrange(0, epho * np.int(numBatches)):
        ...
        _, err_train, y_trainhat, w1, w2 = sess.run([train, loss_train, y_predict, W1, W2], feed_dict=feeddict_train)
        ...
        if step % numBatches == 0:
            i = 0
            print(
            "Iter :%3d, Cross-entropy loss: %4.2f,Classification error: %4.2f " % (step, err_train, classerror_train))
